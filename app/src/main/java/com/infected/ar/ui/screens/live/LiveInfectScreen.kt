...
fun LiveInfectScreen(nav: NavController, appVm: AppViewModel, liveVm: LiveInfectViewModel = viewModel()) {
    ...
    Box(Modifier.fillMaxSize()) {
        ...
        Canvas(Modifier.fillMaxSize()) {
            drawRect(
                brush = Brush.verticalGradient(
                    colors = listOf(Color.Transparent, Color.Red.copy(alpha = overlayAlpha))
                )
            )
        ...
        Column(
            ...
            Text("Stage: ${liveVm.infectionStage}")
            if (liveVm.infecting) LinearProgressIndicator(modifier = Modifier.padding(bottom = 4.dp))
